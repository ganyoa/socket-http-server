...
def response_method_not_allowed():
    """Returns a 405 Method Not Allowed response
       if browser tries to send a post or delete request an error will be raised
    """
...
def parse_request(request):
    ...
    # splitting the request into lines, taking the first line and splitting on space character to get method, path, and HTTP version
    method, path, version = request.split("\r\n")[0].split(" ")

    # return the path only if method is GET, then make use in the server method
    if method != "GET":
...
def response_path(path):
    ...
    # under webroot.
    file_path = os.path.join('./webroot', path[1:])
    ...
    # if path leads to a directory store the files in 'contents'
    if os.path.isdir(file_path):
        mime_type = mimetypes.types_map['.txt'].encode()
        content = "\r\n".join(os.listdir(file_path)).encode()
    # if file, read the contents
    ...
    try:
        while True:
            ...
            try:
                ...
                # responsible for retrieving the request from the client
                request = ''
                while True:
                    data = conn.recv(1024)
                ...
                try:
                    ...
                    response = response_ok(
                        body=content, #b"Welcome to my web server",
                        mimetype=mime_type #b"text/plain"
                    )
